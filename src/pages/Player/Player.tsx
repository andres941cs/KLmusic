    ...
    return(
        <div className='flex flex-col h-full overflow-hidden relative puff-in-center'>
            ...
            <div className='flex items-center bg-background h-20 w-full gap-1 z-50 px-2'>
                {
                    playing ? <PauseIcon onClick={handlePlayPause} className="cursor-pointer fill-foreground hover:fill-primary"/> : <PlayIcon onClick={handlePlayPause} className="cursor-pointer fill-foreground hover:fill-primary"/>
                }
                <span>{format(playedSeconds)}</span>
                {/* <!-- Seekbar --> */}
                <div className='w-4/6 sm:w-full'><Slider min={0} max={100} step={1} value={[played*100]} onValueChange={handleSeekChange}/></div>
                <span className='text-card-foreground'>{format(duration-1)}</span>
                {
                muted ? <MutedIcon onClick={handleToggleMuted} className="cursor-pointer fill-foreground hover:fill-primary"/> : <VolumeIcon onClick={handleToggleMuted} className="cursor-pointer fill-foreground hover:fill-primary"/>
                }
                  <div className='w-12 sm:w-24'><Slider min={0} max={100} step={1}  value={[volume*100]} onValueChange={handleVolumeChange}></Slider></div>
                  <MovieIcon onClick={()=>setVideo(!video)} className="cursor-pointer fill-foreground hover:fill-primary"/>
            </div>
        </div>
    )
}
