...
import { searchSongs } from "@services/SongData";
...
const SearchSong = ({ onData,isOpen,setIsOpen }: ISearchSong) => {
        ...
        if(event.target.value.length >= 3) {
            searchSongs(event.target.value).then((data) => {
                const suggestions = data.map((song:Song) => {
                    return <li className="p-2 hover:bg-muted-foreground dark:hover:bg-muted" key={song.id} onClick={()=>showLyrics(song.id!)}>{song.name}</li>
                })
    ...
    return ( 
        ...
        <div className="absolute z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            ...
            className="border border-white bg-white/5 w-60 h-8 px-3 py-5 outline-none text-white" placeholder="Song" />
            <ul className="border border-t-0 text-white">{sugesstions}</ul>
            <Link to={'/forms'} className="absolute right-0 bottom-full hover:text-primary text-white">Not Found?</Link>
        </div>
        </>
     );
}
